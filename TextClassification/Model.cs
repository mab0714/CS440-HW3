...
namespace TextClassification
{
    public class Model
    {
        ...
        private Dictionary<string, int> _wordCountsByClass = new Dictionary<string, int>();
        ...
        public void Train()
        {
            ...
            foreach (string doc in docs)
            {
                ...
                foreach (string word in words.Split(' ').ToList())
                {
                    ...
                    if (_wordCountsByClass.ContainsKey(cls))
                    {
                        _wordCountsByClass.TryGetValue(cls, out wordCntByClass);
                        wordCntByClass = wordCntByClass + wordCnt;
                        _wordCountsByClass.Remove(cls);
                        _wordCountsByClass.Add(cls, wordCntByClass);
                    }
                    else
                    {
                        _wordCountsByClass.Add(cls, wordCnt);
                    }
                ...
                if (_trainingData.ContainsKey(cls))
                {
                    ...
                    // class dictionary has been updated, remove the class and read it with the updated dictionary
            ...
            if (this._type.Equals("MNB"))
            {
                ...
                foreach (string word in this._uniqueWords)
                {
                    ...
                    foreach (KeyValuePair<string, Dictionary<string, int>> kvp in _trainingData)
                    {
                        ...
                        int totalWordsInDocsFromClass = getTotalWordsInDocsFromClass(kvp.Key);
        ...
        private int getTotalWordsInDocsFromClass(string cls)
        {
            return this._wordCountsByClass[cls];
        }
